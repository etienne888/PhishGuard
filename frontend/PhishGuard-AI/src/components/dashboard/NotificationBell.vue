...
<template>
  <div ref="root" class="relative">
    <button class="relative rounded-xl border border-slate-200 bg-white p-2.5 text-slate-600 transition hover:border-blue-300 hover:text-blue-600"
            :aria-label="t('notifications.bellAria', { n: store.unreadCount })" @click="isOpen = !isOpen">
    ...
    <div v-if="isOpen" class="absolute right-0 z-40 mt-2 w-80 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
      <div class="flex items-center justify-between border-b border-slate-100 px-4 py-3">
        <span class="text-sm font-bold text-slate-800">{{ t('dashboard.tab.notifications') }}</span>
        <button v-if="store.unreadCount" class="text-xs text-blue-600 hover:underline" @click="store.markAllRead()">{{ t('dashboard.markAllRead') }}</button>
      </div>
